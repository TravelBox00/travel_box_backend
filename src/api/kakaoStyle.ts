...
function createAuthorization(accessKey: string, secretKey: string, query: string): string {
  const signedDate = Date.now().toString(); 
  query = query.replace(/\s+/g, " ");
  const message = `${signedDate}.${query}`;
  const hash = crypto.createHmac("sha1", secretKey).update(message).digest("hex");
...
async function fetchZigzagProduct(productId: string) {
  ...
  try {
    ...
    console.log("✅ Zigzag API Response:", JSON.stringify(response.data, null, 2));
  } catch (error: any) {
    console.error("❌ Error fetching Zigzag API:", error.response?.data || error.message);
  }
}
