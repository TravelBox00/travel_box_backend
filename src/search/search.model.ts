...
export const searchValidSuggestions = async (word: string): Promise<string[]> => {
  try {
    ...
    const suggestedRegions = regionResponse.hits.hits.flatMap((hit: any) => hit._source.region_hierarchy);

    const suggestedPosts = postResponse.hits.hits.map((hit: any) => hit._source.postTitle);
