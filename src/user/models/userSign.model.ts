...
export const userInfoDeleteByUserTag = async (userTag: string): Promise<number> => {
    ...
    try {
        ...
        await connection.query(`UPDATE Comment SET userId = NULL WHERE userId = (SELECT userId FROM User WHERE userTag = ?)`, [userTag]);
        await connection.query(`UPDATE \`Like\` SET userId = NULL WHERE userId = (SELECT userId FROM User WHERE userTag = ?)`, [userTag]);

        // 사용자 삭제 (마지막에 수행)
        const [deleteUser]: any = await connection.query(`DELETE FROM User WHERE userTag = ?`, [userTag]);
